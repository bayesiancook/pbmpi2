...
bool compentry(bpentry e1, bpentry e2)	{
...
class UnrootedBPP	{
	...
	UnrootedBPP(string filename, double incutoff)	{
	...
	int ProposeFullGibbsSPR(Tree* tree, Link*& down, Link*& up)	{
		...
		Link* fromdown = 0;
		Link* fromup = 0;
		tree->Detach(subtree,subtreeup,fromdown,fromup);
		GetSummedLogProb(tree,subtree,down,up);
		tree->Attach(subtree,subtreeup,down,up);
