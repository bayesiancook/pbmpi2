...
void RASPARTGTRGammaPhyloProcess::SlaveExecute(MESSAGE signal)	{
...
void RASPARTGTRGammaPhyloProcess::SlaveUpdateParameters()	{
	...
	L1 = GetNmodeMax();
	L2 = GetDim();
	nd = 2*Npart + nbranch + Npart*nrr + L2 + L1*(L2+1);
	if (empmix == 2)	{
		nd += 2;
	}
	ni = 1 + GetNsite();
	...
	MPI_Bcast(dvector,nd,MPI_DOUBLE,0,MPI_COMM_WORLD);
	int index = 0;
	SetRateParams(&dvector[index],&dvector[index+Npart]);
	index +=2*Npart;
	for(i=0; i<nbranch; ++i) {
...
void RASPARTGTRGammaPhyloProcess::ReadPB(int argc, char* argv[])	{
...
void RASPARTGTRGammaPhyloProcess::ReadModeProfiles(string name, int burnin, int every, int until)	{
...
void RASPARTGTRGammaPhyloProcess::SlaveComputeCVScore()	{
	...
	for (int k=0; k<GetNcomponent(); k++)	{
		for (int i=sitemin; i<sitemax; i++)	{
			ExpoConjugatePartitionGTRFiniteProfileProcess::alloc[i] = k;
		}
...
void RASPARTGTRGammaPhyloProcess::SlaveComputeSiteLogL()	{
