...
class PartitionGTRMixtureProfileProcess : public virtual PartitionGTRProfileProcess, public virtual MatrixMixtureProfileProcess {

	public:

	PartitionGTRMixtureProfileProcess() {}
	virtual ~PartitionGTRMixtureProfileProcess() {}
	...
	double GetNormRate(int k)	{
		...
		for (int i=0; i<GetDim(); i++)	{
			for (int j=i+1; j<GetDim(); j++)	{
				tot += rr[k][rrindex(i,j,GetDim())] * profile[k][i] * profile[k][j];
			}
		}
		return 2*tot;
	}
