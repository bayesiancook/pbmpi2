...
void MultiGeneAASubSelRASCATSBDPGammaPhyloProcess::Create()	{
	
	AASubSelRASCATSBDPGammaPhyloProcess::Create();
	MultiGenePhyloProcess::Create();
	MultiGeneAASubSelMixtureProfileProcess::Create();
	if (GetMyid())	{
		for (int gene=0; gene<Ngene; gene++)	{
			if (genealloc[gene] == myid)	{
				process[gene] = new AASubSelRASCATSBDPGammaPhyloProcess(Ncat,kappaprior);
...
void MultiGeneAASubSelRASCATSBDPGammaPhyloProcess::Delete()	{
	if (GetMyid())	{
		for (int gene=0; gene<Ngene; gene++)	{
			if (genealloc[gene] == myid)	{
				delete process[gene];
	...
	MultiGeneAASubSelMixtureProfileProcess::Delete();
	MultiGenePhyloProcess::Delete();
	AASubSelRASCATSBDPGammaPhyloProcess::Delete();
}

double MultiGeneAASubSelRASCATSBDPGammaPhyloProcess::Move(double tuning)	{
...
void MultiGeneAASubSelRASCATSBDPGammaPhyloProcess::GlobalUpdateParameters() {
...
void MultiGeneAASubSelRASCATSBDPGammaPhyloProcess::SlaveUpdateParameters() {
...
void MultiGeneAASubSelRASCATSBDPGammaPhyloProcess::SlaveExecute(MESSAGE signal)	{
	...
	if (! ret)	{
		switch(signal) {
			...
			case COLLECTKAPPAS:
				...
				break;
			default:
			AASubSelRASCATSBDPGammaPhyloProcess::SlaveExecute(signal);
		}
	}
}
