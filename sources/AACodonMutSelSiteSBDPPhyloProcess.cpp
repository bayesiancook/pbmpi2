...
void AACodonMutSelSiteSBDPPhyloProcess::SlaveUpdateParameters()	{
...
void AACodonMutSelSiteSBDPPhyloProcess::SlaveExecute(MESSAGE signal)	{
...
void AACodonMutSelSiteSBDPPhyloProcess::GlobalUpdateParameters() {
...
void AACodonMutSelSiteSBDPPhyloProcess::SlaveComputeCVScore()	{
	...
	for (int k=0; k<GetNcomponent(); k++)	{
		for (int i=sitemin; i<sitemax; i++)	{
			AACodonMutSelSiteSBDPProfileProcess::alloc[i] = k;
		}
		UpdateComponent(k);
...
void AACodonMutSelSiteSBDPPhyloProcess::ReadPB(int argc, char* argv[])	{
...
void AACodonMutSelSiteSBDPPhyloProcess::ReadMapStats(string name, int burnin, int every, int until){
...
void AACodonMutSelSiteSBDPPhyloProcess::Read(string name, int burnin, int every, int until)	{
	...
	int Nstate = AACodonMutSelSiteSBDPSubstitutionProcess::GetNstate();
		...
		for (int site=0; site<ProfileProcess::GetNsite(); site++)	{
			...
			for (int s=0; s<Nstate; s++)	{
				stat[s] = 	GetNucStat(GetCodonStateSpace()->GetCodonPosition(0, s)) *
						...
						codonprofile[s] *
						profile[alloc[site]][GetCodonStateSpace()->Translation(s)];
				Z += stat[s];
			}
			for (int s=0; s<Nstate; s++)	{
				stat[s] /= Z;
				...
				for (int codonTo = 0; codonTo<Nstate; codonTo++)	{
					...
					if ((pos != -1) && (pos != 3))  {
						...
						if (!GetCodonStateSpace()->Synonymous(codonFrom, codonTo))	{
							int aaFrom = GetCodonStateSpace()->Translation(codonFrom);
							int aaTo = GetCodonStateSpace()->Translation(codonTo);
...
int AACodonMutSelSiteSBDPPhyloProcess::CountNonSynMapping()	{
...
int AACodonMutSelSiteSBDPPhyloProcess::CountNonSynMapping(int i)	{
	int count = 0;
...
int AACodonMutSelSiteSBDPPhyloProcess::GlobalNonSynMapping()	{
...
void AACodonMutSelSiteSBDPPhyloProcess::SlaveNonSynMapping()	{
